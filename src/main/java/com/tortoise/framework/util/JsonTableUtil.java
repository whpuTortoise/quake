...
import com.tortoise.framework.paginter.domain.JsonTable;
...
public class JsonTableUtil {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void sendJson(HttpServletResponse response, List jsonList, int total, Object callBack) {
		PrintWriter out = null;
		try {
			response.setContentType("application/json; charset=utf-8");
			response.setHeader("Cache-Control", "no-cache");
			...
			out = response.getWriter();
			if (jsonList != null) {
				JsonTable jsonTable = new JsonTable(jsonList, total, callBack);
				String json = toJson(jsonTable);
				out.print(json);
			}
			out.flush();
